    ...
    F: low_level_il::function::FunctionForm,
{
    ...
    SetReg(low_level_il::LowLevelILRegister<A::Register>, Expression<'a, A, M, F>),
        ...
        low_level_il::expression::ExpressionHandler<'a, A, M, F>,
{
    fn from(expr: LowLevelILExpression<'a, A, M, F, ValueExpr>) -> Self {
        ...
        match expr.kind() {
			...
			Kind::CmpE(operation) => Expression::CmpE(Box::new(BinaryExpression(operation.left().into(), operation.right().into()))),
